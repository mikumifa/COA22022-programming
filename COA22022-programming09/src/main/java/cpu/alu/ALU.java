...
public class ALU {
    ...
    public DataType mul(DataType src, DataType dest) {
        ...
        String product = BinaryIntegers.ZERO + destStr;//前面全是0
        int Y1 = 0;
        int Y2 = product.charAt(2 * length - 1) - '0';//最低位
        for (int i = 0; i < length; i++) {
            switch (Y1 - Y2) {
                case 1:
                    product = adder(product.substring(0, length), X, '0', length) + product.substring(length);//前32位相加用加号来拼接
                    break;
                case -1:
    ...
    private String oneAdder(String operand) {
        ...
        for (int i = 0; i < len; i++) {
            bit = num[i] ^ carry;//get bit
            carry = num[i] & carry;//git carry
            res[i] = (char) ('0' + bit);//don't change the number
        }
    ...
    private String adder(String operand1, String operand2, char c, int length) {
        operand1 = impleDigits(operand1, length);//算数扩展到length位
